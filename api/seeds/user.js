...
const maleNames = ["James", "John", "Robert", "Michael", "William", "David", "Richard","hippo"];
...
const generateBio = () => {
	const descriptors = bioDescriptors.sort(() => 0.5 - Math.random()).slice(0, 6);
...
const generateRandomUser = (gender, index) => {
	...
	return {
		firstName,
		emailId: `${firstName.toLowerCase()}${age}${Math.floor(Math.random() * (45 - 21 + 1) + 21)}@example.com`,
